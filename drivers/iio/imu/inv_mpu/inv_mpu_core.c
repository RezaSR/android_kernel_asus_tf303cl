...
#define MPU6500_IOC_MAGIC 0xF2
#define MPU6500_IOC_MAXNR 2
#define MPU6500_POLL_DATA _IOR(MPU6500_IOC_MAGIC,2,int )

#define MPU6500_IOCTL_START_HEAVY 2
#define MPU6500_IOCTL_START_NORMAL 1
#define MPU6500_IOCTL_END 0

#define START_NORMAL    msecs_to_jiffies(200)  //5HZ
#define START_HEAVY     msecs_to_jiffies(5)    //200HZ

static void  MPU6500_stress_test_poll(struct work_struct * work);
static int stress_test_poll_mode=0;
struct delayed_work MPU6500_stress_test_poll_work;
static struct workqueue_struct *MPU6500_stress_test_work_queue;
static struct inv_mpu_iio_s *sst;
/* define for i2c stress test*/
 ...
 */
#if 0 //For Intel I2C multiple blockes issue
...
//define for node get_mpu65xx_status
//				  get_mpu65xx_gyro_rawdata
//                get_mpu65xx_gsensor_rawdata
//                get_mpu65xx_gcompass_rawdata
#ifdef INV_RAW_DATA_VIA_I2C

static ssize_t get_mpu65xx_status(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct inv_mpu_iio_s *st = iio_priv(dev_get_drvdata(dev));
	int mpu6500_wia = 0;
	uint8_t buffer[1];
	...
	err = inv_i2c_read(st, REG_WHOAMI, 1, &(buffer[0]));
	if(err < 0)
...
#ifdef INV_RAW_DATA_VIA_I2C
static DEVICE_ATTR(status, S_IRUGO, get_mpu65xx_status, NULL);
		...
		const struct i2c_device_id *id)
{
	...
	if (!strcmp(id->name, "itg3500"))
		...
		st->chip_type = INV_MPU6050;
	else if (!strcmp(id->name, "MPU6500:00"))
		st->chip_type = INV_MPU6500;
	else if (!strcmp(id->name, "INVN6500:00"))
		st->chip_type = INV_MPU6500;
...
static long
MPU6500_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	/* for i2c stress test */
	int err = 1;
	if (_IOC_TYPE(cmd) != MPU6500_IOC_MAGIC)
	return -ENOTTY;
	if (_IOC_NR(cmd) > MPU6500_IOC_MAXNR)
	return -ENOTTY;
	...
	switch (cmd) {
		case MPU6500_POLL_DATA:
			if (arg == MPU6500_IOCTL_START_HEAVY){
				printk("gsensor mpu6500 : ioctl heavy\n");
				stress_test_poll_mode = START_HEAVY;
				queue_delayed_work(MPU6500_stress_test_work_queue, &MPU6500_stress_test_poll_work,
						stress_test_poll_mode);
			}
			else if (arg == MPU6500_IOCTL_START_NORMAL){
				printk("gsensor mpu6500 : ioctl normal\n");
				stress_test_poll_mode = START_NORMAL;
				queue_delayed_work(MPU6500_stress_test_work_queue, &MPU6500_stress_test_poll_work,
						stress_test_poll_mode);
			}
			else if  (arg == MPU6500_IOCTL_END){
				printk("gsensor mpu6500 : ioctl end\n");
				cancel_delayed_work_sync(&MPU6500_stress_test_poll_work);
			}
			else
				return -ENOTTY;
			break;
...
static struct miscdevice mpu_dev = {
	...
	const struct i2c_device_id *id)
{
	...
	if (!id) {
		printk("inv_mpu id is NULL\n");
		return -ENODEV;
	}
printk("enter inv_mpu_probe addr=%d irq=%d id->name=%s\n", client->addr, client->irq, id->name);
	...
	if (indio_dev == NULL) {
		printk("inv_mpu memory allocation failed\n");
		result =  -ENOMEM;
		goto out_no_free;
	}
...
//configure gpio pin for mpuirq
	result = gpio_request(st->plat_data.int_gpio,"mpu6500_int");
	if (result < 0)
		printk("inv_mpu gpio_request fail err =%d \n",result);
	result = gpio_direction_input(st->plat_data.int_gpio);
	if (result < 0)
		printk("inv_mpu gpio_direction_input fail err =%d \n",result);
	st->client->irq = gpio_to_irq(st->plat_data.int_gpio);
	printk("inv_mpu probe client irq(%d)\n", st->client->irq);
#endif
...
if( (!strcmp(id->name, "MPU6500:00"))||(!strcmp(id->name, "INVN6500:00"))){
	strcpy(indio_dev->name,"mpu6500");//notice it's lower case letter for ml_sys_helper.c name matching
        printk("wlin MPU Remapping id->name to mpu6500\n");
}
	...
	if (result) {
		printk("inv_mpu inv_mpu_configure_ring fail\n");
		goto out_free;
	}
	result = iio_buffer_register(indio_dev, indio_dev->channels,
					indio_dev->num_channels);
	if (result) {
		printk("ring iio_buffer_registerfail\n");
		goto out_unreg_ring;
	}
	st->irq = client->irq;
	result = inv_mpu_probe_trigger(indio_dev);
	if (result) {
		printk("inv_mpu inv_mpu_probe_trigger fail\n");
	...
	if (result) {
		printk("inv_mpu iio_device_register fail\n");
	...
	if (INV_MPU6050 == st->chip_type ||
		...
		if (result) {
			printk("inv_mpu inv_create_dmp_sysfs failed\n");
			goto out_unreg_iio;
		}
	}
	...
	result = misc_register(&mpu_dev);
	if (result) {
		dev_err(&client->dev,
			"%s: mpu_dev register failed", __func__);
		goto destroy_stress_test_wq;
	}

	sst = st;
	MPU6500_stress_test_work_queue = create_singlethread_workqueue("i2c_mpu6500_wq");
	if(!MPU6500_stress_test_work_queue){
		printk("gsensor mpu6500 : unable to create i2c stress test workqueue\n");
		goto destroy_stress_test_wq;
	}
	INIT_DELAYED_WORK(&MPU6500_stress_test_poll_work, MPU6500_stress_test_poll);
...
destroy_stress_test_wq:
	if(MPU6500_stress_test_work_queue)
		destroy_workqueue(MPU6500_stress_test_work_queue);
out_unreg_iio:
...
/***** i2c stress test *****/
static int MPU6500_stress_test_get_gyro_rawdata(struct inv_mpu_iio_s *st){
...
static int MPU6500_stress_test_get_gsensor_rawdata(struct inv_mpu_iio_s *st){
...
static void  MPU6500_stress_test_poll(struct work_struct * work)
{
	...
	MPU6500_stress_test_get_gyro_rawdata(sst);
	MPU6500_stress_test_get_gsensor_rawdata(sst);
	if(stress_test_poll_mode == 0)
		msleep(5);
	queue_delayed_work(MPU6500_stress_test_work_queue, &MPU6500_stress_test_poll_work, stress_test_poll_mode);
}
...
static int inv_mpu_remove(struct i2c_client *client)
{
	...
	if (misc_deregister(&mpu_dev) < 0)
		dev_err(&client->dev, "misc deregister mpu_dev failed.");
...
static const struct i2c_device_id inv_mpu_id[] = {
	...
	{"mpu6050", INV_MPU6050},
	{"mpu9150", INV_MPU9150},
	{"MPU6500:00", INV_MPU6500},
	{"INVN6500:00", INV_MPU6500},
	{"mpu6500", INV_MPU6500},
    {"mpu6515", INV_MPU6500},
	{"mpu9250", INV_MPU9250},
	{"mpu6xxx", INV_MPU6XXX},
...
struct acpi_device_id mpu6500_acpi_match[] = {
		{MPU_I2C_NAME,0},
};
...
static struct i2c_driver inv_mpu_driver = {
	...
	.driver = {
		...
		.name	=	MPU_I2C_NAME,
		.pm     =       INV_MPU_PMOPS,
		.acpi_match_table = ACPI_PTR(mpu6500_acpi_match),
	},
