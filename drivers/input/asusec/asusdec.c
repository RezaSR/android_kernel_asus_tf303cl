...
/*303CL define*/
static unsigned int asusdec_dock_in_gpio =2;
static unsigned int asusdec_dock_in_gpio1 =0; //DOCK_IN_1#
static unsigned int asusdec_dock_in_gpio_irq = 0;
static unsigned int asusdec_dock_in1_gpio_irq = 0;
static unsigned int asusdec_apwake_gpio =154;
static unsigned int asusdec_apwake_gpio_irq = 0;//get from apwake_gpio
//gpio GPIO1P[3] from 200 to 215 in PMIC
static unsigned int asusdec_dock_power =138; 
...
//	ec_chip->dock_init = 0;
	...
	intel_scu_ipc_iowrite8(GPIO1P3, 0x20);
	msleep(20);
	intel_scu_ipc_iowrite8(GPIO1P3, 0x21);
}
//[Brook- Fix bug 295323 + System can't wakeup with dcoking when insert/removed AC]<<
...
static irqreturn_t asusdec_interrupt_handler(int irq, void *dev_id){
    ...
    if(!ec_chip->dockin_id1_enable)
		ec_chip->dockin_id1_status = 1;
    if(irq==asusdec_dock_in_gpio_irq)
		ec_chip->dockin_id2_status = 1;
	else if(irq==asusdec_dock_in1_gpio_irq)
		ec_chip->dockin_id1_status = 1;
	if(irq==asusdec_dock_in_gpio_irq && ec_chip->dockin_id1_status){
		queue_delayed_work(asusdec_wq, &ec_chip->asusdec_dock_init_work, 0);
	}
	else if(irq==asusdec_dock_in1_gpio_irq && ec_chip->dockin_id2_status){
		queue_delayed_work(asusdec_wq, &ec_chip->asusdec_dock_init_work, 0);
	}
...
static int asusdec_irq_ec_request(struct i2c_client *client)
{
	...
	//open the ec_request in PMIC
	rc = intel_scu_ipc_iowrite8(GPIO1P3, 0x20);//set open cmos 0x21
	//print the pmic status
#ifdef ASUSDEC_DEBUG_MESSAGE
	rc = intel_scu_ipc_ioread8(GPIO1P3, &value);
	if (rc)
		printk("read of %x failed, err %x\n",GPIO1P3, rc);
	else
		printk("read of %x success, %x\n", GPIO1P3, value);
#endif
	return 0 ;
...
static int asusdec_irq_dock_in(struct i2c_client *client)
{
	...
	unsigned gpio1 = asusdec_dock_in_gpio1;
	//set dock in irq for irq handler
	unsigned irq =asusdec_dock_in_gpio_irq=gpio_to_irq(asusdec_dock_in_gpio);
	unsigned irq1 =asusdec_dock_in1_gpio_irq=gpio_to_irq(asusdec_dock_in_gpio1);
	...
	rc = gpio_request(gpio1, label);
	if (rc) {
		ASUSDEC_ERR("gpio_request failed for input id1=%d\n", gpio1);
	}
	...
	rc = gpio_direction_input(gpio1) ;
	if (rc) {
		ASUSDEC_ERR("gpio_direction_input failed for input id1=%d\n", gpio1);
		goto err_gpio_direction_input_failed;
	}
	ASUSDEC_INFO("GPIO DOCK_IN_1# = %d , state = %d\n", gpio1, gpio_get_value(gpio1));
	//request dock power
	rc = gpio_request(dock_pwr_gpio,"DOCK_PWR_EN");//138
	if (rc) {
		ASUSDEC_ERR("gpio_request failed for input %d\n", dock_pwr_gpio);
	}

	if (!gpio_get_value(gpio) && !gpio_get_value(gpio1)){
	...
	if(Read_HW_ID() >= HW_ID_PR){
	   ASUSDEC_NOTICE("Read_HW_ID = %d\n",Read_HW_ID());
	   ec_chip->dockin_id1_enable = 1;
       rc = request_irq(irq1, asusdec_interrupt_handler,IRQF_SHARED|IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING/*|IRQF_TRIGGER_HIGH|IRQF_TRIGGER_LOW*/, label, client);
	   if (rc < 0) {
		   ASUSDEC_ERR("Could not register for %s interrupt, irq = %d, rc = %d\n", label, irq1, rc);
		   rc = -EIO;
		   goto err_gpio_request_irq_fail ;
...
static void asusdec_send_ec_req(void){
	...
	intel_scu_ipc_iowrite8(GPIO1P3, 0x21);//set open cmos 0x21	
#ifdef ASUSDEC_DEBUG_MESSAGE
	rc = intel_scu_ipc_ioread8(GPIO1P3, &value);//read pmic
	if (rc)
...
static void asusdec_dock_init_work_function(struct work_struct *dat)
{
	...
	int gpio1 = asusdec_dock_in_gpio1;
	int gpio_state = 0;
	int gpio_state1 = 0;
	int retry = 20;
	int rc =0;
    ...
    gpio_state1 = gpio_get_value(gpio1);
	printk("dock in gpio state = %d\n",gpio_state);
	printk("dock in gpio state1 = %d\n",gpio_state1);
