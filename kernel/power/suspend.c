	...
	for_each_irq_desc(irq, desc) {
		...
		if (desc->istate & IRQS_PENDING) {
                        if (desc->action && desc->action->name) {
                            switch (irq) {
                                  case 8:
                                      printk(KERN_DEBUG "Resume caused by WAKE18, alarm by IRQ%d %s\n", irq, desc->action->name);
                                      break;
                                  case 401:
                                      printk(KERN_DEBUG "Resume caused by WAKE32, wifi by IRQ%d %s\n", irq, desc->action->name);
                                      break;
                                  case 402:
                                      printk(KERN_DEBUG "Resume caused by WAKE%d, by IRQ%d %s\n", irq, irq,desc->action->name);
                                      break;
                                  case 67:
                                      printk(KERN_DEBUG "Resume caused by WAKE%d, by IRQ%d %s\n", irq, irq,desc->action->name);
                                      break;
