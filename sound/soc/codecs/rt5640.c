...
#define RT5640_WAKEUP_DELAY	400
#define USE_EQ
#define USE_ASRC
#define VERSION "0.8.4 alsa 1.0.25"

// i2c stress test ++
#define RT5640_IOC_MAGIC 0xF3
#define RT5640_IOC_MAXNR 2
#define RT5640_POLL_DATA _IOR(RT5640_IOC_MAGIC,2,int )

#define RT5640_IOCTL_START_HEAVY 2
#define RT5640_IOCTL_START_NORMAL 1
#define RT5640_IOCTL_END 0

#define START_NORMAL    (HZ)
#define START_HEAVY     (HZ)

static int poll_mode = 0;
static int codec_reset = 203;
struct delayed_work rt5640_poll_data_work;
static struct workqueue_struct *sensor_work_queue;
struct i2c_client *rt5640_client;
struct miscdevice rt5640_misc_dev;
...
/*	{RT5640_I2S1_SDP	, 0xD000},//change IIS1 and IIS2*/
	...
	{RT5640_IN3_IN4, 0x0400},	/*IN2 boost 35db and signal ended mode */
...
/*	{RT5640_REC_R2_MIXER	, 0x007d},//Mic1 -> RECMIXR*/
	...
	{RT5640_ADC_DIG_VOL, 0xe2e2},
        {RT5640_ADC_BST_VOL, 0x0000},   /*DMIC boost gain 0 db*/
/*	{RT5640_MONO_MIXER, 0xcc00},*/	/*OUTMIX -> MONOMIX */
        //{RT5640_DIG_INF_DATA, 0x4000}, /*swap right/left channel*/
        {RT5640_DRC_AGC_1, 0x2206},
        {RT5640_DRC_AGC_2, 0x1fa6},
        {RT5640_DRC_AGC_3, 0x210e},
#if IS_ENABLED(CONFIG_SND_SOC_RT5642)
			      ...
			      unsigned int reg, unsigned int value)
{
	...
	if (ret < 0) {
		dev_err(codec->dev, "Failed to set private addr %d for write: %d\n", reg, ret);
		goto err;
	}
	ret = snd_soc_write(codec, RT5640_PRIV_DATA, value);
	if (ret < 0) {
		dev_err(codec->dev, "Failed to set private value %d: %d\n", value, ret);
		goto err;
	}
	return 0;
				      ...
				      unsigned int reg)
{
	...
	ret = snd_soc_write(codec, RT5640_PRIV_INDEX, reg);
	if (ret < 0) {
		dev_err(codec->dev, "Failed to set private addr %d for read: %d\n", reg, ret);
		return ret;
	}
				    ...
				    unsigned int value)
{
	...
	if (ret < 0) {
		dev_err(codec->dev, "Failed to read private reg for update: %d\n", ret);
		goto err;
	}
	...
	if (change) {
		...
		if (ret < 0) {
			dev_err(codec->dev,
				"Failed to write private reg for update: %d\n", ret);
			goto err;
		}
	}
			    ...
			    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		/*Headset AMIC*/
		snd_soc_write(codec, RT5640_DRC_AGC_1, 0xc206); /*MX-B4=c206*/
		snd_soc_write(codec, RT5640_DRC_AGC_2, 0x1fa4); /*MX-B5*/
		snd_soc_write(codec, RT5640_DRC_AGC_3, 0x210e); /*MX-B6*/
		break;

	case SND_SOC_DAPM_POST_PMD:
		snd_soc_write(codec, RT5640_DRC_AGC_1, 0x0206); /*disable DRC*/
		snd_soc_write(codec, RT5640_DRC_AGC_2, 0x1fa6); /*MX-B5*/
		snd_soc_write(codec, RT5640_DRC_AGC_3, 0x210e); /*MX-B6*/
		rt5640_index_update_bits(codec,
...
#ifdef USE_EQ
		rt5640_update_eqmode(codec, DMIC);
#endif
		snd_soc_write(codec, RT5640_DRC_AGC_1, 0x0206); /*MX-B4=0206*/
		
		break;
	case SND_SOC_DAPM_POST_PMD:
#ifdef USE_EQ
		rt5640_update_eqmode(codec, DMIC_STOP);
#endif
		break;
	default:
		return 0;
...
static int rt5640_probe(struct snd_soc_codec *codec)
{
	...
	if (ret != 0) {
        ...
        // TC_Hsu : Add for ATD audio_codec_status
        ret_codec_status = 1;
        pr_debug("rt5640_probe end");
        INIT_DELAYED_WORK(&hp_amp_work, do_hp_amp);

        if(gpio_request(codec_reset,"rt5640_probe"))
        {
            pr_debug("Fail to request gpio pin CODEC_RESET_N");
            codec_reset = 0;
        }
...
static int rt5640_suspend(struct snd_soc_codec *codec)
{
	 ...
	 */
        pr_debug("rt5640_dsp_suspend +");
	rt5640_dsp_suspend(codec);
#endif
        pr_debug("%s()\n", __func__);
        //rt5640_set_bias_level(codec, SND_SOC_BIAS_OFF);
        //if(codec_reset > 0) gpio_direction_output(codec_reset, 0);
        //pr_debug("CODEC_RESET_N pull low to power off rt5640");
	return 0;
}
...
static int rt5640_resume(struct snd_soc_codec *codec)
{
        ...
        //if(codec_reset > 0) gpio_direction_output(codec_reset, 1);
        //pr_debug("CODEC_RESET_N pull high to power on rt5640");
        //msleep(RT5640_WAKEUP_DELAY);
#if IS_ENABLED(CONFIG_SND_SOC_RT5642)
	/* After opening LDO of codec, then close LDO of DSP. */
	rt5640_dsp_resume(codec);
        pr_debug("rt5640_dsp_resume -");
#endif
        pr_debug("%s()-\n", __func__);
	return 0;
}
 ...
 * i2c stress test ++
 */
int rt5640_open(struct inode *inode, struct file *filp)
{
	pr_debug("baytrail audio info : %s\n", __func__);
	return 0;
}

int rt5640_release(struct inode *inode, struct file *filp)
{
	pr_debug("baytrail audio info : %s\n", __func__);
	return 0;
}

int rt5640_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int err = 1;

	if (_IOC_TYPE(cmd) != RT5640_IOC_MAGIC)
	return -ENOTTY;
	if (_IOC_NR(cmd) > RT5640_IOC_MAXNR)
	return -ENOTTY;
	...
	switch (cmd) {
		case RT5640_POLL_DATA:
			if (arg == RT5640_IOCTL_START_HEAVY){
				pr_debug("baytrail audio info : ioctl heavy\n");
				poll_mode = START_HEAVY;
				queue_delayed_work(sensor_work_queue, &rt5640_poll_data_work, poll_mode);
			}
			else if (arg == RT5640_IOCTL_START_NORMAL){
				pr_debug("baytrail audio info : ioctl normal\n");
				poll_mode = START_NORMAL;
				queue_delayed_work(sensor_work_queue, &rt5640_poll_data_work, poll_mode);
			}
			else if  (arg == RT5640_IOCTL_END){
				pr_debug("baytrail audio info : ioctl end\n");
				cancel_delayed_work_sync(&rt5640_poll_data_work);
			}
			else
				return -ENOTTY;
			break;
...
struct file_operations rt5640_fops = {
	.owner = THIS_MODULE,
	.unlocked_ioctl = rt5640_ioctl,
	.open =	rt5640_open,
	.release = rt5640_release,
};

static void  rt5640_poll_data(struct work_struct * work)
{
	//int lux = rt5640_get_adc_value(rt5640_client);
	//pr_debug("FOR TEST , rt5640_poll_data baytrail audio lux = %d\n",lux);
        int lux = snd_soc_read(rt5640_codec, 0);
	pr_debug("FOR TEST , rt5640_poll_data baytrail audio = %d\n",lux);

        //add for Monkey 
        lux = snd_soc_read(rt5640_codec, RT5640_SPK_L_MIXER);
	pr_debug("FOR Monkey , rt5640_poll_data baytrail audio reg RT5640_SPK_L_MIXER = %d\n",lux);
        lux = snd_soc_read(rt5640_codec, RT5640_SPK_R_MIXER);
	pr_debug("FOR Monkey , rt5640_poll_data baytrail audio reg RT5640_SPK_R_MIXER = %d\n",lux);
				      ...
				      const struct i2c_device_id *id)
{
	...
	sensor_work_queue = create_singlethread_workqueue("i2c_baytrailaudio_wq");
	if(!sensor_work_queue){
		pr_err("rt5640_probe: Unable to create workqueue");
		goto exit_i2c;
	}

	INIT_DELAYED_WORK(&rt5640_poll_data_work, rt5640_poll_data);
	rt5640_client = i2c;
	rt5640_misc_dev.minor  = MISC_DYNAMIC_MINOR;
	rt5640_misc_dev.name = "baytrailaudio";
	rt5640_misc_dev.fops = &rt5640_fops;
	ret = misc_register(&rt5640_misc_dev);
	if (ret){
		pr_err("baytrail audio err : Unable to register %s misc device\n",
				rt5640_misc_dev.name);
		goto exit_i2c;
	}
        //--
exit_i2c:
...
static int rt5640_i2c_remove(struct i2c_client *i2c)
{
        ...
        //struct rt5640_data *data = i2c_get_clientdata(i2c);
        misc_deregister(&rt5640_misc_dev);
