...
hweq_t hweq_param[] = {
	...
	{			/* SPK */
	 ...
	 ,
	 {0x1c10, 0x0000, 0xc1dc, 0x1e46, 0xf5ae, 0xcbc1, 0x188d, 0x0699,
	  0xecb0, 0x1279, 0x0424, 0x063c, 0x0fc7, 0x0699, 0x0436, 0x0000,
	  0x1fb4, 0x004b, 0x1fb4, 0x0800, 0x0800}
	 ,
	 0x0000,
	 }
	,
...
int rt5640_update_eqmode(struct snd_soc_codec *codec, int mode)
{
	...
	if (DMIC == eqmode && DMIC_STOP != mode) /*DMIC priority higher than others*/
		return 0;
	...
	if (mode == DMIC){
		snd_soc_update_bits(codec, RT5640_EQ_CTRL1,
			    RT5640_EQ_UPD | RT5640_EQ_SRC_MASK,
			    RT5640_EQ_UPD | RT5640_EQ_SRC_ADC);
	}
	else {
		snd_soc_update_bits(codec, RT5640_EQ_CTRL1,
			    RT5640_EQ_UPD | RT5640_EQ_SRC_MASK,
			    RT5640_EQ_UPD | RT5640_EQ_SRC_DAC);
	}
