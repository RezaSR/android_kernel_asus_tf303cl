...
#define ENABLE_HEADSET_BUTTON		0
     ...
     struct snd_kcontrol *kcontrol, int event)
{
    ...
    if (SND_SOC_DAPM_EVENT_ON(event)) {
        ...
        snd_soc_write(codec, RT5640_DRC_AGC_2, 0x1bb2);
...
*/
static int Ext_Spk_event(struct snd_soc_dapm_widget *w,
     struct snd_kcontrol *kcontrol, int event)
{
    struct snd_soc_dapm_context *dapm = w->dapm;
    struct snd_soc_card *card = dapm->card;
    ...
    codec = byt_get_codec(card);
    if (SND_SOC_DAPM_EVENT_ON(event)) {
        snd_soc_write(codec, RT5640_DIG_INF_DATA, 0x4000);
    } else if(SND_SOC_DAPM_EVENT_OFF(event))
    {
        snd_soc_write(codec, RT5640_DIG_INF_DATA, 0x0000);
    }
...
static const struct snd_soc_dapm_widget byt_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SPK("Ext Spk", Ext_Spk_event),
...
static int snd_byt_mc_probe(struct platform_device *pdev)
{
	...
	//hs_gpio[BYT_CODEC_GPIO_IDX].gpio = codec_gpio;
        ...
        // 20140122: gpio set to 139. Because this is not correctly initialized,
        //           acpi would return default value, and that's incorrect.
	pr_debug("%s: force GPIOs - codec %d", __func__, 139);
	hs_gpio[BYT_CODEC_GPIO_IDX].gpio = 139;
        jd_gpio = -1; 
