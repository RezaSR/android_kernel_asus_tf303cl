...
static struct i2c_board_info __initdata hpa_i2c_device = {
	I2C_BOARD_INFO("hpa02235", 0x55),
};
*/
static struct i2c_board_info __initdata macallan_i2c_asusdec_info[]= {
	{
		I2C_BOARD_INFO("asuspec", 0x1b),
	},
};
...
static int __init asusdec_i2c_init(void) {
        int ret = 0;
        printk("===================  asusdec_i2c_init    =====================\n");
	...
	i2c_register_board_info(3,macallan_i2c_asusdec_info,ARRAY_SIZE(macallan_i2c_asusdec_info));	
	if (ret < 0) {
		pr_err("%s: unable to register asusdec (%d)\n", __func__, ret);
		return ret;
	}
...
//module_init(bq27520_i2c_init);
module_init(asusdec_i2c_init);
