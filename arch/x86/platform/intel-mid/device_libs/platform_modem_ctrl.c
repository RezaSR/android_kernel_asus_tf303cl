...
void *retrieve_acpi_modem_data(struct platform_device *pdev)
{
	...
	if (mcd_reg_info->conf_type == XMM_CONF_M2) {
		...
		cpu_data->gpio_sim1_cd = 117;
		cpu_data->gpio_ap_req_cdump = 51;

		pr_info("%s:Setup GPIOs(WWAN_Disable:%d, W_WWAN:%d, RB:%d, SIM1_CD:%d, AP_CDUMP:%d)",
			__func__,
			cpu_data->gpio_wwan_disable,
			cpu_data->gpio_wake_on_wwan,
			cpu_data->gpio_rst_bbn,
			cpu_data->gpio_sim1_cd, cpu_data->gpio_ap_req_cdump);
	} else {
		cpu_data->gpio_pwr_on =
			acpi_get_gpio_by_index(&pdev->dev, 0, NULL);
		cpu_data->gpio_cdump =
		...
		pr_info("%s:Setup GPIOs(PO:%d, RO:%d, RB:%d, CD:%d, SIM1_CD:%d, AP_CDUMP:%d)",
			__func__,
			cpu_data->gpio_pwr_on,
			cpu_data->gpio_rst_out,
