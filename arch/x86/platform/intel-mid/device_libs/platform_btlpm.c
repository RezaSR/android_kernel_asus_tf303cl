...
static int __init bluetooth_init(void)
{
	...
	bcm_bt_lpm_pdata.gpio_enable = get_gpio_by_name("BT_DEV_EN");
...
#ifdef LPM_ON
	bcm_bt_lpm_pdata.gpio_host_wake = get_gpio_by_name("BT_HOST_WAKE");
	...
	bcm_bt_lpm_pdata.gpio_wake = get_gpio_by_name("BT_DEV_WAKE");
	if (!gpio_is_valid(bcm_bt_lpm_pdata.gpio_wake)) {
		pr_err("%s: gpio %s not found\n", __func__, "BT_DEV_WAKE");
		return -ENODEV;
	}
